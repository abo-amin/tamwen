...
[Authorize]
public class FreeSalePricesController : Controller
{
    private readonly AppDbContext _db;
    private readonly ICurrentStoreProvider _currentStoreProvider;

    public FreeSalePricesController(AppDbContext db, ICurrentStoreProvider currentStoreProvider)
    {
        _db = db;
    ...
    {
        return RedirectToAction("Index", "Prices", new { yearMonth, type = UiTabs.FreeSalePrice });
    }

    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Save(FreeSaleMonthlyPricesViewModel model)
    {
        var vm = new UnifiedMonthlyPricesViewModel
        {
            YearMonth = model.YearMonth,
            PriceType = UiTabs.FreeSalePrice,
            Rows = (model.Rows ?? new List<FreeSaleMonthlyPriceRowVm>())
    ...
    {
        return await new PricesController(_db, _currentStoreProvider).CopyFromPrevious(yearMonth, UiTabs.FreeSalePrice);
    }
