...
try {
    ...
    // Look for the specific nav block
    const searchStr = '<nav class="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200/70 navbar-glass md:hidden" aria-label="التنقل السريع">';
    const endIndex = content.indexOf('</nav>', content.indexOf(searchStr));

    if (content.indexOf(searchStr) !== -1 && endIndex !== -1) {
        console.log('Found navigation block.');
        const oldBlock = content.substring(content.indexOf(searchStr), endIndex + 7);
        
        const newBlock = `    @if (User?.Identity?.IsAuthenticated == true)
    {
        <nav class="fixed bottom-4 left-4 right-4 z-50 md:hidden overflow-hidden rounded-3xl border border-indigo-100 bg-gradient-to-r from-indigo-600 via-indigo-500 to-violet-500 shadow-xl" aria-label="التنقل السفلي">
        ...
        console.log('Could not find the navigation block in _Layout.cshtml. Search result:', content.indexOf(searchStr));
    }
} catch (e) {
